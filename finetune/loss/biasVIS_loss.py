...
class fusion_bias_VIS_loss(nn.Module):
    def __init__(self):
        super(fusion_bias_VIS_loss, self).__init__()
    ...
    def forward(self, image_infrared, image_visible, image_fused):
        ...
        loss_int = (self.loss_func_Intensity(image_visible, image_infrared, image_fused))
        loss_text =  self.loss_func_Grad(image_visible_gray, image_infrared_gray, image_fused_gray)
        loss_color =  self.loss_color(image_visible, image_fused)
        total_loss =  10 * loss_int + 12 * loss_color + 5 * loss_text
        return total_loss
...
class L_Intensity(nn.Module):
    ...
    def forward(self, image_visible, image_infrared, image_fused):
        image_intensity = image_visible # torch.max(image_visible, image_infrared)
        Loss_intensity = F.l1_loss(image_intensity, image_fused) + 0.5 * F.l1_loss(image_infrared, image_fused)
